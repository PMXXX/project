...
#读入excel文件模块
ObjExcel = pd.read_excel('xl3.xlsx')
ObjExcel.head()
Excel = xlrd.open_workbook("xl3.xlsx")
    ...
    if ColResult_1 == 'False':
        if ColResult_6 == 'False':
            if ColResult_7 == 'False':
                print("主要数据没有缺失可导入数据库")
                print("################")
                print("正在导入数据库")
                print("################")
                database = MySQLdb.connect (host="localhost",user = "root",passwd="YES",db = "Prefume",use_unicode=True,charset="utf8")#建立数据库链接，配置这里即可
                ...
                for LineResult in Line_6IsNull:
                    ...
                    if LineResult == 'True':
                        print("分子式第%d个缺失，请检查后重新录入数据库"%LineNo)
                    LineNo = LineNo + 1
        else: #检查SIMELS数据那些缺失
            ...
            for LineResult in Line_5IsNull:
                LineResult = str(LineResult)
                if LineResult == 'True':
                    print("SMILES第%d个缺失，请检查后重新录入数据库"%LineNo)
                LineNo = LineNo + 1
    else:#检查香料名称那些数据缺失
        ...
        for LineResult in LineIsNull:
            LineResult = str(LineResult)
            if LineResult == 'True':
                print("香料名称第%d个缺失，请检查后重新录入数据库"%LineNo)
            LineNo = LineNo + 1
